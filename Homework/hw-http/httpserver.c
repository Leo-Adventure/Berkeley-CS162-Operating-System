...
void serve_file(int fd, char *path, long size)
{
  ...
  /* PART 2 BEGIN */
  char size_str[100];
  snprintf(size_str, 100, "%ld", size);
  printf("The size_str is %s\n", size_str);
  ...
  int byte_read = 0;
  char *buf = (char *)calloc(MAX_BUFF + 1, sizeof(char));

  // read the file
  while ((byte_read = read(file, buf, MAX_BUFF)) > 0)
  {
    // write to the content part of fd
    http_send_string(fd, buf, byte_read);
  }
  close(file);
...
void serve_directory(int fd, char *fn)
{
  struct dirent *entry;
  DIR *dir;
  int count;
  ...
  struct stat info;
  int has_html = 0;
  char *html = (char *)calloc(100, sizeof(char));
  char *buffers[1024];
  int cnt = 0;
  http_format_index(html, fn);
  ...
  else
  {
    while ((entry = readdir(dir)) != NULL)
    {
      ...
      strcpy(path, fn);
      if (strcmp((char *)entry->d_name, "index.html") == 0)
      {
        has_html = 1;
        break;
      }
      ...
      {
        buffers[cnt] = (char *)calloc(strlen(path) + 100, sizeof(char));
        http_format_href(buffers[cnt], path, entry->d_name);
        strcat(buffers[cnt], "\n");
      }
      cnt++;
    }
    if (has_html)
    {
      if (stat(html, &info) == 0 && S_ISREG(info.st_mode))
      {
        serve_file(fd, html, info.st_size);
      }
    }
    else
    {
      ...
      for (int i = 0; i < cnt; i++)
      {
        http_send_string(fd, buffers[i], strlen(buffers[i]));
      }
    }
...
void handle_files_request(int fd)
{
  ...
  path = (char *)calloc(1, 2 + strlen(request->path) + 1);
  path[0] = '.';
  path[1] = '/';
  ...
  if (stat(path, &file_stat) == 0)
  {
    ...
    {
      // printf("Invoking serve_file\n");
      serve_file(fd, path, file_stat.st_size);
    }
...
void handle_proxy_request(int fd)
{
  ...
  while (1)
  {
    ...
    int fd_final = (fd > target_fd) ? fd : target_fd;
    fd_final++;

    select(fd_final, &rset, NULL, NULL, NULL);
    ...
    if (FD_ISSET(fd, &rset))
    {
      ...
      http_send_string(target_fd, req_buffer, nread);
    }

    if (FD_ISSET(target_fd, &rset))
    {
      if ((nread = read(target_fd, proxy_buffer, MAX_BUFF)) == 0)
      {
        if (client_eof == 1)
        {
          break;
      ...
      http_send_string(fd, proxy_buffer, nread);
    }
  }
  close(target_fd);
  free(req_buffer);
...
void serve_forever(int *socket_number, void (*request_handler)(int))
{
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  while (1)
  {
    ...
    int ret = pthread_create(&new_thread, NULL, request_handler, (void*) client_socket_number);
